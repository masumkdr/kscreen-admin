...
const Genres = () => {
  const [data, setData] = useState(GenresData);
...
useEffect(() => {
  const stored = localStorage.getItem('genres_data');
  if (stored && stored !== 'undefined') {
    setData(JSON.parse(stored));
  } else {
    setData(GenresData);
    localStorage.setItem('genres_data', JSON.stringify(GenresData));
  }
}, []);
  ...
  const fields = [
    { name: 'name', label: 'Genre Name', type: 'text' },
  ];
  ...
  const handleDelete = (row) => {
    if (window.confirm(`Delete location "${row.name}"?`)) {
      ...
      showMessage('info', 'Genre deleted successfully');
    }
  };
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      ...
      showMessage('success', 'Genre updated successfully');
    } else {
  ...
  return (
    <>
      ...
      <DialogForm
        ...
        onSave={handleSave}
        title={editItem ? 'Edit Genre' : 'Add Genre'}
      >
        <FormBuilder
          fields={fields}
          values={formValues}
